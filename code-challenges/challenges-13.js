...
const objectCounter = (obj) => {
    let count=1;
let res=obj.reduce(value=>count=count+1);
...
const stringReverse = (str) => {
    let array=str.split(" ");
 let result=array.reduce((pre,aft)=>{
     return aft+" "+pre;
...
const statistics = (obj) => {
    let obj_result={};
   let result=obj.reduce((pre,aft)=>{
       if(pre!=null)
       obj_result[pre.votes_To]=1;
       if(obj_result[aft.votes_To]==null){
           obj_result[aft.votes_To]=1;
       }
       else{
           obj_result[aft.votes_To]+=1;
       }
